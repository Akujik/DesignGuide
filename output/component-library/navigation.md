...
| 组件类型 | 用途 | 特殊功能 | 响应式支持 |
|----------|------|----------|------------|
| Header | 网站主导航 | 下拉菜单、用户操作 | 完全响应式 |
| Sidebar | 应用内导航 | 用户信息、快速操作 | 移动端折叠 |
| Breadcrumb | 层级导航 | 路径指示、快速返回 | 移动端截断 |
| Tabs | 内容切换 | 键盘导航、动画过渡 | 水平滚动 |
| Pagination | 分页导航 | 省略号显示、跳转功能 | 完全响应式 |
| Steps | 流程导航 | 进度指示、步骤验证 | 移动端垂直布局 |
